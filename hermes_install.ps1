...
function Write-Banner {
    ...
    Write-Host "│  镜像脚本版本：2026.04.14+9c7027a                                       │" -ForegroundColor Magenta
    Write-Host "│  最后更新：2026-04-14 22:35:25 CST                                        │" -ForegroundColor Magenta
    Write-Host "└────────────────────────────────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Python {
    ...
    if (Get-Command python -ErrorAction SilentlyContinue) {
        ...
        Write-Info "如果你后续仍安装失败，请到 [设置 > 应用 > 应用执行别名] 里关闭 python.exe / python3.exe。"
    }
    ...
    Write-Info "如果你之前见过 Microsoft Store 的 Python 提示，建议顺手到 [设置 > 应用 > 应用执行别名] 里关闭 python.exe / python3.exe。"
    return $false
}
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        $mirrorZipUrls = @("$MirrorBaseUrl/hermes-agent-$Branch.zip")
        if ($Branch -ne "main") {
            $mirrorZipUrls += "$MirrorBaseUrl/hermes-agent-main.zip"
        }
...
function Install-Venv {
    ...
    & $UvCmd venv venv --python $script:PythonSpecifier
